...
| `tokens/textTokens.ts`          | `text.color` (semantic text colors: `primary`/`secondary`/`muted`/`disabled`/`link`/`danger`/`success`/`warning`/`inverse`) plus size-keyed style presets `text.heading` (`h1`-`h4`), `text.body` (`sm`/`md`/`lg`), `text.caption` (`sm`/`md`), `text.label` (`sm`/`md`) — each preset holds `{ fontSize, fontWeight, lineHeight }` — and `text.decoration`/`text.transform`/`text.overflow`/`text.whiteSpace`/`text.align`. Consumed by `primitives/text`. |
| `tokens/index.ts`               | Barrel: aggregates every token module.                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `models/Theme.ts`               | `CoreUIXTheme` — the canonical theme shape every token section conforms to. Top-level sections: `colors`, `radius`, `spacing`, `shadow`, `zIndex`, `breakpoints`, `border`, `opacity`, `transition`, `width`, `height`, `typography`, `flex`, `sidebar`, `text`.                                                                                                                                                                                            |
...
each top-level section through one of six hand-enumerated constants:
...
- **`FLAT_SECTIONS`** — sections that flatten straight to `--cuix-<prefix>-<kebab-key>` (e.g.
  ...
  `breakpoint`), `width`, `height`, `sidebar`, `opacity`. The CSS prefix per section is looked up
  in `SECTION_CSS_PREFIX`, a required (non-optional) `Record`, so a missing entry is a compile
...
- **`BORDER_SECTIONS`** — the nested sub-sections of `border` (`width`, `style`), flattened via
  `BORDER_CSS_PREFIX` to `--cuix-border-width-*`/`--cuix-border-style-*`.
- **`TRANSITION_SECTIONS`** — the nested sub-sections of `transition` (`duration`, `easing`),
  flattened via `TRANSITION_CSS_PREFIX` to `--cuix-transition-duration-*`/`--cuix-transition-easing-*`.
- **`TEXT_FLAT_SECTIONS`** — the nested sub-sections of `text` that are already flat
  `Record<string, string>` maps (`color`, `decoration`, `transform`, `overflow`, `whiteSpace`,
  `align`), flattened via `TEXT_FLAT_CSS_PREFIX` to e.g. `--cuix-text-color-primary`.
- **`TEXT_PRESET_SECTIONS`** — the nested sub-sections of `text` that are size-keyed style
  presets (`heading`, `body`, `caption`, `label`, each holding `{ fontSize, fontWeight,
lineHeight }` per size key), flattened via `TEXT_PRESET_CSS_PREFIX` to
  `--cuix-<prefix>-<size>-<prop>`, e.g. `--cuix-text-heading-h1-font-size`.
...
`EXCLUDED_SECTIONS` + the top-level keys `"typography"`, `"text"`, `"border"`, `"transition"`
(the sections with nested sub-section constants above are accounted for at the top level, not
per-sub-section) and checks every key of the actual `theme` object against it. If a top-level
`CoreUIXTheme` section exists that isn't in that set, it **throws**
(`generateCssVariables: unhandled theme section(s): ...`) instead of silently dropping it — this
is a safety net added specifically because `flex` used to fall through unnoticed before
`EXCLUDED_SECTIONS` existed. **Practical consequence:** adding a whole new top-level theme
section to `models/Theme.ts` requires adding it to the accounted-for set in
...
- **Add a whole new top-level section**: add it to `models/Theme.ts`, add a `tokens/*.ts` default
  ...
  `SECTION_CSS_PREFIX` entry), a new nested-sub-section constant pair (following the
  `BORDER_SECTIONS`/`TEXT_FLAT_SECTIONS`/etc. shape if the section has nested sub-objects rather
  than flat string values), or `EXCLUDED_SECTIONS` in `generateCssVariables.ts` — and add the
  section's key to the `accountedFor` set. Skipping this step means the app throws at runtime
  instead of silently missing a variable (`text`, `border`, and `transition` were all added this
  way). If it needs Tailwind semantic-class mapping, add a corresponding entry in
  `tailwind.config.ts` too.
