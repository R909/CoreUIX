...
export const cardVariants: ReturnType<typeof cva> = cva(
...
export const cardHeaderVariants: ReturnType<typeof cva> = cva(
  `${flex.col} space-y-[var(--cuix-spacing-tight)] p-[var(--cuix-spacing-lg)]`,
);

// Title text within CardHeader.
export const cardTitleVariants: ReturnType<typeof cva> = cva(
...
export const cardDescriptionVariants: ReturnType<typeof cva> = cva(
  "text-[var(--cuix-font-size-sm)] text-[var(--cuix-colors-muted-foreground)]",
);

// Main body region of the card.
export const cardContentVariants: ReturnType<typeof cva> = cva(
  "p-[var(--cuix-spacing-lg)] pt-0",
);

// Footer row, usually for action buttons.
export const cardFooterVariants: ReturnType<typeof cva> = cva(
